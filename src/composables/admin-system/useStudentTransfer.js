...
import { createStudentTransferDraft, validateStudentLifecycleDraft } from '@/contracts/admin-system/student-profiles'
import { transferStudentProfile } from '@/services/admin-system/studentProfiles'

export function useStudentTransfer(options = {}) {
  const form = reactive(createStudentTransferDraft())
  ...
  function reset(overrides = {}) {
    Object.assign(form, createStudentTransferDraft(overrides))
    fieldErrors.value = {}
    feedback.value = null
  }
  ...
  async function submit(studentProfileId) {
    ...
    pending.value = true
    try {
      const result = await (options.submitter ?? transferStudentProfile)(studentProfileId, form, options.serviceOptions?.())
