...
export function usePasswordResetCompletion({
  token,
  ...
  const resetToken = computed(() => String(toValue(token) ?? '').trim())
  const completed = computed(
    () => feedback.value?.state === ACCOUNT_LIFECYCLE_FEEDBACK_STATES.success,
  )
  const tokenInvalid = computed(() => isMalformedLifecycleToken(resetToken.value))
  ...
  async function submit(input = {}) {
    ...
    feedback.value = null
    try {
      const response = await service.completePasswordReset({
