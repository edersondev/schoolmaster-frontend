...
export function useGuardianContactView({
  route = {},
  context = null,
  ...
  const hasMissingValues = computed(() => {
    const contact = state.contactView
    if (!contact) return false
    return [
      contact.guardianContact?.email,
      contact.guardianContact?.phone,
      contact.studentPrimaryContact?.email,
      contact.studentPrimaryContact?.phone,
    ].some((value) => !value)
  })
  ...
  return { state: readonly(state), studentProfileId, hasMissingValues, load }
}
