export const administrationMessages = {
  common: {
    create: 'Create',
    submit: 'Create',
    cancel: 'Cancel',
    close: 'Close',
    retry: 'Retry',
    search: 'Search',
    resetFilters: 'Reset filters',
    actions: 'Actions',
    view: 'View',
    edit: 'Edit',
    update: 'Update',
    delete: 'Delete',
    status: 'Status',
    allStatuses: 'All statuses',
    active: 'Active',
    planned: 'Planned',
    closed: 'Closed',
    inactive: 'Inactive',
    pageSize: 'Rows per page',
    loading: 'Loading administration records.',
    empty: 'No records exist in this permitted scope.',
    filteredEmpty: 'No records match current filters.',
    forbidden: 'Your permissions do not allow this administration surface.',
    conflict: 'This operation conflicts with the current record state.',
    tenantMismatch: 'Choose an active permitted school to continue.',
    inactiveContext: 'Current school is inactive.',
    inactiveRecord: 'This record is inactive and cannot be changed.',
    unsupportedQuery: 'Some filters or pagination values are not supported.',
    notFound: 'Requested record is unavailable in this permitted scope.',
    unavailable: 'Administration service is temporarily unavailable.',
    sessionExpired: 'Your session expired. Sign in again to continue.',
    signIn: 'Sign in',
    unknownError: 'Administration request failed.',
    validationSummary: 'Correct following fields before submitting.',
    effectiveDate: 'Effective date',
    reason: 'Reason',
    deleteDialogTitle: 'Delete school',
    deleteDialogMessage: 'Delete {name}? Provide the effective date and a reason to confirm.',
    discardTitle: 'Discard unsaved changes?',
    discardMessage: 'Changes on this page will be lost.',
    discardConfirm: 'Discard changes',
    stay: 'Stay',
    success: 'Record created successfully.',
    updateSuccess: 'Record updated successfully.',
    deleteSuccess: 'Record deleted successfully.',
    name: 'Name',
    description: 'Description',
    code: 'Code',
    cnpj: 'CNPJ',
    inepCode: 'INEP code',
    email: 'Email',
    city: 'City',
    state: 'State',
    administrativeType: 'Administrative type',
    legalNature: 'Legal nature',
    managementType: 'Management type',
    pedagogicalApproach: 'Pedagogical approach',
    phone: 'Phone',
    address: 'Address',
    fullName: 'Full name',
    roles: 'Roles',
    permissions: 'Permissions',
    scope: 'Scope',
    startDate: 'Start date',
    endDate: 'End date',
    sequence: 'Sequence',
    academicYear: 'Academic year',
    relationship: 'Relationship',
    students: 'Students',
    select: 'Select',
    previous: 'Previous',
    next: 'Next',
    finish: 'Finish',
    pageOf: 'Page {page} of {count}',
    sort: 'Sort',
    defaultSort: 'Default',
    nameAscending: 'Name A–Z',
    nameDescending: 'Name Z–A',
    emailAscending: 'Email A–Z',
    roleScopeHelp: 'Role scope is fixed to current school.',
    filterAcademicYear: 'Filter by academic year',
  },
  navigation: {
    schools: 'Schools',
    users: 'Users',
    roles: 'Roles',
    permissions: 'Permissions',
    academicYears: 'Academic years',
    academicPeriods: 'Academic periods',
    guardians: 'Guardians',
    schoolDetail: 'School detail',
    userDetail: 'User detail',
    roleDetail: 'Role detail',
    academicYearDetail: 'Academic year detail',
    academicPeriodDetail: 'Academic period detail',
    guardianDetail: 'Guardian detail',
    editRole: 'Edit role',
    editAcademicYear: 'Edit academic year',
    editAcademicPeriod: 'Edit academic period',
    editGuardian: 'Edit guardian',
  },
  schools: {
    title: 'Schools',
    createTitle: 'Create school',
    editTitle: 'Edit school',
    detailTitle: 'School detail',
    invalidFilters: 'Some filters are unavailable and were not applied: {fields}.',
    lookupUnavailable:
      'Institutional filter options are unavailable. Identity and contact filters can still be used.',
  },
  users: {
    title: 'Users',
    createTitle: 'Create user',
    editTitle: 'Edit user',
    detailTitle: 'User detail',
    deleteTitle: 'Delete user',
    deleteMessage: 'Delete {name}? Provide the effective date and a reason to confirm.',
    recovery: {
      warning: 'An existing user can be restored.',
      restoreAction: 'Restore existing user',
      resourceLabel: 'existing user',
    },
  },
  roles: {
    title: 'Roles',
    createTitle: 'Create role',
    editTitle: 'Edit role',
    detailTitle: 'Role detail',
    listPermissions: 'List permissions',
    permissionsDialogTitle: '{role} permissions',
    permissionName: 'Permission name',
    noPermissions: 'This role has no permissions.',
  },
  permissions: { title: 'Permissions' },
  academicYears: {
    title: 'Academic years',
    dateRange: 'Date range',
    dateRangeSeparator: 'to',
    createTitle: 'Create academic year',
    editTitle: 'Edit academic year',
    detailTitle: 'Academic year detail',
  },
  academicPeriods: {
    title: 'Academic periods',
    createTitle: 'Create academic period',
    editTitle: 'Edit academic period',
    detailTitle: 'Academic period detail',
  },
  guardians: {
    title: 'Guardians',
    createTitle: 'Create guardian',
    editTitle: 'Edit guardian',
    detailTitle: 'Guardian detail',
  },
}
