...
const { state, loadList, create, correct } = useGrades({ options: { schoolId: session.activeSchool?.id } })
</script>
...
<template>
  <main class="grid gap-6 p-6">
    <header>
      ...
      <h1 class="text-3xl font-bold text-sm-heading">Grades</h1>
    </header>
    ...
    <section class="grid gap-4 rounded-2xl border border-sm-border bg-sm-surface p-5">
      <ElAlert
        v-if="!state.gates.scopedGradeList"
        title="Scoped grade list blocked"
      ...
      <ElForm label-position="top">
        <div class="grid gap-3 md:grid-cols-2">
          ...
          <ElFormItem label="Grade value" required>
            <ElInputNumber v-model="state.draft.gradeValue" :min="0" :max="100" class="w-full" />
          </ElFormItem>
          <ElFormItem label="Grade label">
            <ElInput v-model="state.draft.gradeLabel" />
          </ElFormItem>
        </div>
        <ElButton type="primary" :loading="state.pending" @click="create">Record grade</ElButton>
      </ElForm>
    </section>
    <section v-if="state.detail" class="grid gap-3 rounded-2xl border border-sm-border bg-sm-surface p-5">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <h2 class="text-xl font-semibold">Current grade: {{ state.detail.currentValue }}</h2>
        <TeacherWorkflowStatusControls :status="state.detail.status" :show-lifecycle="false" />
      </div>
      <ElButton @click="correctionOpen = true">Correct grade</ElButton>
    </section>
