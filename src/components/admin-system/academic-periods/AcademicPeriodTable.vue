...
const columns = [
  ...
  { prop: 'sequence', label: t('administration.common.sequence') },
  { prop: 'startDate', label: t('administration.common.startDate') },
  { prop: 'endDate', label: t('administration.common.endDate') },
...
<template>
  <AdminDataTable :rows="rows" :columns="columns">
    <template #name="{ row }"><button class="font-medium text-sm-brand hover:underline" type="button" @click="$emit('view', row)">{{ row.name ?? '—' }}</button></template>
    <template #select="{ row }"><ElCheckbox v-if="bulkEnabled" :model-value="selectedIds.includes(row.id)" :aria-label="`Select ${row.name ?? row.id}`" @update:model-value="$emit('toggle-selection', { row, checked: $event })" /></template>
