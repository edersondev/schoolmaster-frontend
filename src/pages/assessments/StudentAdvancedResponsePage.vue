...
const response = useStudentAdvancedResponse({ questionnaire, options: access.options.value })
const feedback = computed(() => (access.canSubmit.value.allowed ? response.state.feedback : access.canSubmit.value.feedback))
</script>

<template>
  <section class="space-y-5">
    <AdvancedAssessmentStatusRegion :busy="response.state.pending" :feedback="feedback" />
