...
const detail = useLoadedAcademicRecordDetail('attendance', route)
const isDetail = computed(() => Boolean(route.params.attendanceId))
...
<template>
  <main class="mx-auto w-full max-w-6xl px-4 py-6">
    ...
    <ElButton
      ...
      class="mb-4"
      plain
      @click="router.push({ name: STUDENT_SELF_SERVICE_ROUTE_NAMES.attendance })"
    >
    ...
    <section v-if="isDetail" class="space-y-4">
      ...
      <StudentAcademicRecordDetail :record="detail.record.value" type="attendance" />
    </section>
    <section v-else class="space-y-4" aria-labelledby="student-attendance-title">
      <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <h1 id="student-attendance-title" class="text-2xl font-semibold text-slate-950">
          {{ t('studentSelfService.attendance.title') }}
        </h1>
      ...
      <div class="grid gap-3">
        ...
        >
          <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div>
              <h2 class="text-base font-semibold text-slate-950">{{ record.attendanceDate }}</h2>
              <p class="text-sm text-slate-600">{{ record.attendanceStatus }}</p>
            </div>
          ...
          <ElButton
            ...
            type="primary"
            plain
            @click="router.push({ name: STUDENT_SELF_SERVICE_ROUTE_NAMES.attendanceDetail, params: { attendanceId: record.id } })"
          >
