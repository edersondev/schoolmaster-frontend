...
import {
  ...
  listRoles,
  restoreRole,
} from '@/services/admin-system/roles'
...
const permissionsDialogOpen = shallowRef(false)
const permissionsRole = shallowRef(null)
...
const bulk = useAdminBulkLifecycle({
  ...
  routeName: route.name,
  submitter: (input) => bulkLifecycleRoles(input, { schoolId: tenantId.value }),
  onSuccess: async () => {
    ElMessage.success(t('administration.common.updateSuccess'))
    await list.load(list.query.value)
  },
...
function onView(row) {
  router.push({ name: 'roleDetail', params: { roleId: row.id }, query: route.query })
}
function onEdit(row) {
  router.push({ name: 'roleEdit', params: { roleId: row.id }, query: route.query })
}
function onListPermissions(row) {
...
async function submitLifecycle() {
  try {
    ...
    // Lifecycle composable exposes the mapped error state to the dialog.
  }
}
async function submitBulkLifecycle() {
  try {
    ...
    // Bulk composable exposes the mapped error state to the dialog.
  }
}
</script>
<template>
