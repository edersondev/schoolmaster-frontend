...
function resetFilters() {
  list.updateQuery({
    academicPeriodId: scope.currentPeriod.value?.academicPeriodId ?? '',
    status: '',
  })
}
</script>
  ...
  <main class="space-y-5">
    <header class="flex flex-wrap items-center justify-between gap-3">
      ...
      <ElButton
        type="primary"
        :icon="Plus"
        @click="router.push({ name: 'classSectionCreate', query: $route.query })"
        >Create</ElButton
      >
    </header>
    <div class="rounded-xl border border-sm-border bg-sm-surface p-4">
      <ClassSectionFilters
        :academic-period-id="scope.selectedAcademicPeriodId.value"
